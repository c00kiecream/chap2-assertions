...
public class JupiterAssertionsTests {
    ...
    * 모든 Jupiter Assertions의 메소드는 정직(static) 메소드 형태로 제공하고 있다.
     */

    //give : 테스트 시 필요한 파라미터를 준비하다.
    //when : 테스트할 메소드를 호출한다.
    //then : 실행 결과를 검증한다.
...
//        Assertions.assertEquals(expected, result, 1);
    ...
    /* 목차. 3. assertNull */
    /* 필기. assertNull(actual) 메소드는 레퍼런스 변수가 null 값을 갖는지를 판단한다. */
    @Test
...
//        Assertions.assertTrue(result);
    ...
    /*목차. 7.assertAll */
    /* 필기. assertAll(Executable...) 모든 Assertion이 실행되고 실패가 함게 보고되는 그룹화된 Assertion */
    @Test
